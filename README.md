...
### 📊 Архитектура системы Analyst Machine (Мета-когнитивная система)
  
#### 🧠 Центр когнитивной маршрутизации ()
- Определение контекста задачи
- Выбор шаблона мышления
- Запуск мыслительного процессора
- Управление итерациями и глубиной анализа

#### ⚙️ Интеллектуальный процессор мышления 

> *(ИПМ — когнитивный маршрутизатор системы)*

- Входные данные → ключевой запрос
- Тип мышления (структурность, глубина, исследовательскость)
- Подключение шаблона
- Динамическая адаптация метода
- Проверка вывода (целостность, достоверность, логика)

#### 📜 Универсальный перечень шаблонов (`Core_Template_List`)
1. Итерационный анализ
2. Диалоговое исследование
3. Концептуальный разбор
4. Интерактивный вызов
5. Потоковый анализ
6. Адаптивное погружение

#### 🔁 План итераций (`Iteration_Plan`)
- Построение сценария ответа (тема → итерации)
- Разделение на блоки и логические фазы
- Навигация по итерациям с подтверждением
- Возможность гибкой работы в процессе погружения
- Широкое применение: от учебной деятельности до глубокого анализа узкий тем

#### 🎨 Правила оформления текста (`Text_Styling`)
- Выбор стиля (академический, аналитический, креативный и др.)
- Визуальные маркеры (🧠, 📊, 📌 и др.)
- Правила зонирования, акцентов, форматирования
- Генерация читабельного и логичного текста

#### 📘 Принципы взаимодействия (`Core_Principles`)
- Принципы мышления и диалога
- Принцип «антиутечки» (не раскрывать ядро)
- Прозрачность структуры при сохранении логической капсулы
- Научиность, достоверность и объективность

#### 🛠 Внутренний журнал изменений (`Z_Changelog`)
- Фиксация наблюдений и предложений по архитектуре
- Улучшения и противоречия в логике
- Интеграция новых приёмов без внешнего отображения
